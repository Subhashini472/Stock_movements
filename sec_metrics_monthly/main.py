...
SOURCE_TABLE = "clariversev1.flipkart_slices.sec_voice_metrics_daily"
TARGET_TABLE = "clariversev1.flipkart_slices.sec_voice_metrics_monthly"
...
def get_last_processed_month_end():
    query = f"""
        SELECT MAX(window_end_ts)
        FROM `{TARGET_TABLE}`
        WHERE window_type = 'MONTH'
    """
...
def align_to_month(ts: datetime) -> datetime:
    return ts.replace(day=1, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc)
...
def run_monthly_aggregation():
    ...
    last_processed_end = get_last_processed_month_end()
    if last_processed_end is None:
        print("❌ No daily source data available.")
        return

    current_month_start = align_to_month(last_processed_end)

    # DO NOT align now_ts
    now_ts = datetime.now(timezone.utc)

    if current_month_start >= now_ts:
        print("✅ No new monthly windows to process.")
        return
    ...
    WITH monthly_base AS (
        SELECT
            DATE(TIMESTAMP_TRUNC(window_start_ts, MONTH)) AS month_date,
            TIMESTAMP_TRUNC(window_start_ts, MONTH) AS month_start,
            TIMESTAMP_ADD(
                TIMESTAMP_TRUNC(window_start_ts, MONTH),
                INTERVAL 1 MONTH
            ) AS month_end,
            *
        FROM `{SOURCE_TABLE}`
    ...
    aggregated AS (
        SELECT
            ...
            MAX(escalation_requested_count) AS max_daily_escalations,
            AVG(escalation_requested_count) AS avg_daily_escalations

        FROM monthly_base
        GROUP BY month_start, month_end, month_date
    ),
    ...
    with_trend AS (
        SELECT
            *,
            escalation_risk_pct
              - LAG(escalation_risk_pct) OVER (ORDER BY month_start)
              AS escalation_risk_trend,

            SAFE_DIVIDE(max_daily_escalations, avg_daily_escalations)
