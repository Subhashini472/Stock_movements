...
TARGET_TABLE = "clariversev1.flipkart_slices.sec_voice_metrics_daily"
...
def get_last_processed_day_end():
    query = f"""
...
def align_to_day(ts: datetime) -> datetime:
...
def run_daily_aggregation():
    ...
    last_processed_end = get_last_processed_day_end()
    if last_processed_end is None:
        print("❌ No hourly source data available.")
        return

    current_day_start = align_to_day(last_processed_end)

    # ✅ FIX: do NOT align now_ts
    now_ts = datetime.now(timezone.utc)

    if current_day_start >= now_ts:
        print("✅ No new daily windows to process.")
        return
    ...
    WITH daily_base AS (
        SELECT
            DATE(window_start_ts) AS day_date,
            TIMESTAMP_TRUNC(window_start_ts, DAY) AS day_start,
            TIMESTAMP_ADD(TIMESTAMP_TRUNC(window_start_ts, DAY), INTERVAL 1 DAY) AS day_end,
            *
        FROM `{SOURCE_TABLE}`
        WHERE window_type = 'HOUR'
    ),
    ...
    aggregated AS (
        ...
        FROM daily_base
        GROUP BY day_start, day_end, day_date
    ),
    ...
    with_trend AS (
        SELECT
            *,
            escalation_risk_pct
              - LAG(escalation_risk_pct) OVER (ORDER BY day_start)
